...
- **Thaw Day:** A separate coordinated change with one height per network, `Consensus::Params::nDDThawDayHeight`. Every network in this source ships with it unscheduled (`std::numeric_limits<int>::max()`); regtest can set it with `-ddthawdayheight=N`, which is a startup error elsewhere (`src/kernel/chainparams.cpp:203, 554, 1006, 1281-1283`). Read it through `DigiDollar::IsThawDayActive(params, candidate_height)`.
